        ...
        {songs.map((song) => (
          <div key={song.id} className="bg-[#232328] p-4 rounded-lg shadow-md relative">
            ...
            <div className="flex items-center justify-center gap-12 mt-4">
              <button onClick={() => handlePlayPause(song.songCid)} className="flex items-center gap-2">
              ...
              <CustomPaper>
                ...
                <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
                  ...
                  <Stack direction='row' spacing={1}
                    ...
                    <FaFastBackward style={{ color: 'white', cursor: 'pointer' }} onClick={() => audioPlayer.current!.currentTime -= 10} />

                    {!isPlaying
                      ? <FaPlay fontSize={'large'} style={{ color: 'white', cursor: 'pointer' }} onClick={() => { audioPlayer.current!.play(); setIsPlaying(true); }} />
                      : <FaPause fontSize={'large'} style={{ color: 'white', cursor: 'pointer' }} onClick={() => { audioPlayer.current!.pause(); setIsPlaying(false); }} />
                    }

                    <FaFastForward style={{ color: 'white', cursor: 'pointer' }} onClick={() => audioPlayer.current!.currentTime += 10} />
                  </Stack>
...
function VolumeBtns({ volume, mute, setMute }: VolumeBtnsProps) {
  return mute
    ? <FaVolumeOff style={{ color: 'white', cursor: 'pointer' }} onClick={() => setMute(!mute)} />
    : volume <= 20 ? <FaVolumeMute style={{ color: 'white', cursor: 'pointer' }} onClick={() => setMute(!mute)} />
      : volume <= 75 ? <FaVolumeDown style={{ color: 'white', cursor: 'pointer' }} onClick={() => setMute(!mute)} />
        : <FaVolumeUp style={{ color: 'white', cursor: 'pointer' }} onClick={() => setMute(!mute)} />
}
