...
const AdminTokenCard = (props: Props) => {
    ...
    const [tokenId] = useState<BigNumber>(BigNumber.from(Number(props.tokenId)));
    ...
    return (
        <div className="w-80 h-[450px] rounded-lg flex flex-col overflow-hidden shadow-lg hover:shadow-xl transition-shadow duration-300 relative group">
            ...
            <div className="w-full flex">
                <button className="w-full text-center py-2 bg-[#5b5bd5] font-semibold text-lg text-white cursor-pointer" onClick={handleOpenDialog}>
                    Details
                </button>
            </div>
            ...
            {isDialogOpen && (
                <div className='fixed inset-0 z-50 bg-black bg-opacity-50 flex items-center justify-center'>
                    <div className='bg-[#232328] p-8 rounded-lg text-white w-1/3 relative'>
                        <IoClose className='absolute top-4 right-4 cursor-pointer text-white' size={24} onClick={handleCloseDialog} />
                        <h2 className='text-2xl font-semibold mb-4 text-white'>Token Details</h2>
                        <div className='mb-4'>
                            <h3 className='text-lg font-semibold text-white'>Step 1: Artist Token Sales</h3>
                            <button className='bg-[#5b5bd5] text-white px-4 py-2 rounded-lg mt-2' onClick={handleTokenSales}>
                                Complete Token Sales
                            </button>
                        </div>
                        <div className='mb-4'>
                            <h3 className='text-lg font-semibold text-white'>Step 2: Release Revenue</h3>
                            <button className='bg-[#5b5bd5] text-white px-4 py-2 rounded-lg mt-2' onClick={handleRevenue}>
                                Release Revenue
                            </button>
                        </div>
                        <div className='mb-4'>
                            <h3 className='text-lg font-semibold text-white'>Step 3: Distribute Revenue</h3>
                            <button className='bg-[#5b5bd5] text-white px-4 py-2 rounded-lg mt-2' onClick={handleDistribute}>
                                Distribute Revenue
                            </button>
                        </div>
                        <div className='flex justify-end'>
                            <button className='bg-[#5b5bd5] text-white px-4 py-2 rounded-lg' onClick={handleCloseDialog}>
                                Close
                            </button>
                        </div>
                    </div>
